...
drops=c("taxlevel","rankID","daughterlevels","total")
bacteria.df.6=bacteria.df.6[,!(names(bacteria.df.6)%in% drops)]
venn.df=t(bacteria.df.6)
venn.df=as.data.frame(venn.df)
v.names=rownames(venn.df)
rownames(venn.df)=NULL
venn.df=cbind(v.names,venn.df)
colnames(venn.df)=as.character(unlist(venn.df[1,]))
venn.df=venn.df[-1,]
names(venn.df)[1]="Sample"
taxa.names=list(colnames(venn.df))
sample.names=list(venn.df[,1])

binary.tax="initialize"
binary.tax=for i in seq_along(sample.names){
  for j in seq_along(taxa.names){
    if ([i,j]>0){
      binary.tax[i,j]=paste0
    }
  }
}

test=apply(venn.df,1,function(x) which(x>0))

venn.test=cbind(venn.df$Sample,as.character(test))
venn.test=as.data.frame(venn.test)
```
