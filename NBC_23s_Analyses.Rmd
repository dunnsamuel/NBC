---
title: "NBC 23s Analyses.`r format(Sys.time(), '%d %B, %Y')`"
author: "Sam Dunn"
...
```{r Envfit}
env.df<- read.csv("C:/Users/sdunn3/Google Drive/North Branch Chicago Data/NBC/env.df") #env data from my study.
env.drops=c("08/15/17","08/22/17","08/25/17","09/01/17","09/08/17","09/12/17") #trimmign sample dates where I didn't sequence
...
ord.dist=vegdist(algae.df.sps) #get distances
ord.nmds=metaMDS(ord.dist) #run nmds
ORD=data.frame(MDS1=ord.nmds$points[,1],MDS2=ord.nmds$points[,2]) #exrtact nmds coordinates
ORD=cbind(ORD,env.df.fac) #combine nmds coordinates with env data

fit=envfit(ord.nmds,env.df.fit,perm=999,na.rm=T,display="sites") #correlate you nmds coordintes with env data  Higher perm number means mroe sensitive p value
fit
...
spp.scrs=spp.scrs[ which (spp.scrs$pvals<0.05),] #select only significant covariates
...
ggplot(ORD)+
  ...
  geom_text(data = spp.scrs, aes(x = NMDS1/5, y = NMDS2/5, label = Species),
            size = 3) +
    geom_path(data=df_ell, aes(x=MDS1, y=MDS2,colour=group), size=1, linetype=2)+ggtitle("Algal Communities")+theme_classic() #I adjuested the length of the arrows (NMDS1/5, etc) so they woudl fit better int he plot.  This is purely for aesthtetic pruposes
...
venn(list("Hard"=Hard,"Soft"=Soft,"Sheet"=Sheet,"Foam"=Foam,"Tile"=Tile))
